...
看老六的那篇windows上安装nodejs的文章：[windows下安装nodejs和fnm](https://class0x0e.github.io/blog/how-to-download-nodejs/)
...
如果你的pr有问题我会在pr下面留言，修改不需要新开pr，只需要在本地继续修改文件，然后再次执行 `git add`、`git commit` 和 `git push`。新的改动会自动同步到当前的 PR 中。

#### 查看action

每次pr合并之后，github action会自动部署博客网站。你可以在你的fork仓库页面的“Actions”标签页下查看action的运行状态。如果失败了，建议先自己排查一下，把报错信息发给ai，解决不了欢迎骚扰我。
...
文章头image字段指定的封面放在`public/images/posts/`目录下，作者头像放在`public/images/authors/`目录下。

文章内的图片资源建议放在`src/content/assets/`目录下，文章内引用图片时使用相对路径引用，相对于`src/content/posts/`目录，比如：

```markdown
![图片描述](../assets/your-image.jpg)
```

../表示上一级目录，即`src/content/`，再加上`assets/your-image.jpg`就是图片的完整路径了。

#### 视频资源的存放

文章内的视频资源建议放在`public/videos/`目录下，这个博客解析的时候是直接把`src`字段加到`videos/`后面的，因此 **只用填文件名**，比如：

```markdown
<Video src="2025.mp4" title="视频标题" width={1000} height="auto" />
```

其中，`width`和`height`可以根据需要调整。`width=1000`大小适中，`height="auto"`可以保持视频的原始宽高比。

<Notice type="warning">
视频文件建议控制在几十MB以内，过大的视频会导致博客加载缓慢，过大的视频可能超出github单次上传限制（100MB）。
</Notice>

#### bug上报

如果在使用过程中发现bug，欢迎到[博客仓库的issues页面](https://github.com/class0x0E/class0x0e.github.io/issues) 提交issue，描述清楚bug现象和复现步骤，我会尽快修复。或者直接找我反馈也可以。
